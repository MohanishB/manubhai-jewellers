...
class _MJMultiSelectChipsDropdownFieldState<T extends Object>
  ...
  void _unfocusEverything() {
    // ✅ IMPORTANT: closes keyboard even if some other TextField was focused
    FocusManager.instance.primaryFocus?.unfocus();
  }
   
  @override
  void didUpdateWidget(covariant MJMultiSelectChipsDropdownField<T> oldWidget) {
    super.didUpdateWidget(oldWidget);

    if (_open && _overlayEntry != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        // widget may have been disposed / overlay closed
  ...
  void _toggle() {
    ...
    _unfocusEverything(); // ✅ fix keyboard staying open
...
void _toggleValue(T v) {
    ...
    // ✅ rebuild overlay now (safe because this runs in response to tap)
    _overlayEntry?.markNeedsBuild();
    if (mounted) setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    // ✅ This widget has NO TextField, so it can never open keyboard.
...
class _ChipsRow<T extends Object> extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // ✅ Fix #1: plain hint text (no rounded pill)
    if (values.isEmpty) {
      return Align(
...
class _RectChip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // ✅ Fix #2: better chip (like screenshot) — rectangle + border + X
    return Container(
      height: 28,
